...
class SAM2Base(torch.nn.Module):
    ...
    # 这个函数是sam_decoder的入口函数，也就是说我要在这个backbone_features输入前去对最后一层做一个新模块
    # 不对，应该在image_encoder后面添加（也就是加在encoer里）会比较方便并且不会破坏
    ...
    def forward_image(self, img_batch: torch.Tensor):
        ...
        '''
        return {
            "image_embed": backbone_out["vision_features"], # 最终的 64x64 特征
            "high_res_feats": high_res_features,            # 细节特征列表
        }
        '''
